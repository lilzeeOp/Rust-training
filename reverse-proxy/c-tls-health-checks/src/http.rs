...
/// Parses an HTTP/1.x request and rewrites headers for proxy forwarding.
/// Injects: Host (upstream), X-Forwarded-For, X-Forwarded-Proto: https,
///          X-Request-ID, Via, Connection: close.
/// Strips client-supplied versions of all injected headers to prevent spoofing.
pub fn build_forwarded_request(
    ...
    // Headers we control — strip client-supplied versions to prevent spoofing
    let strip = [
        "host", "connection",
...
mod tests {
    ...
    #[test]
    fn test_injects_x_forwarded_proto_https() {
        let req = make_request("GET", "/", &[("Host", "example.com")]);
        let (out, _, _) = parse(&req);
        let s = String::from_utf8(out).unwrap();
    ...
    #[test]
    fn test_drops_spoofed_x_forwarded_proto() {
        let req = make_request("GET", "/", &[
            ("Host", "example.com"),
        ...
        let s = String::from_utf8(out).unwrap();
        assert_eq!(
            s.matches("X-Forwarded-Proto").count(), 1,
            "exactly one X-Forwarded-Proto"
        );
        assert!(!s.contains("evil"), "spoofed proto must be dropped");
    }
